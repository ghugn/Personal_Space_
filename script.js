document.addEventListener('DOMContentLoaded', () => {
    ...
    const firebaseConfig = {
        apiKey: "AIzaSyDEJospgVQil2WaBQLkBYklSXyFZvlsLsI",
        authDomain: "focus-space-4f792.firebaseapp.com",
        projectId: "focus-space-4f792",
        storageBucket: "focus-space-4f792.firebasestorage.app",
        messagingSenderId: "928932406196",
        appId: "1:928932406196:web:d54d35a39917d5972408e1"
    };
    ...
    // --- AUTH LOGIC ---
    const customGoogleLogin = document.getElementById('custom-google-login');
    ...
    const navSettings = document.querySelector('[data-target="view-settings"]');

    // =============================================
    // CHROME EXTENSION GOOGLE AUTH (Implicit Grant Flow)
    // =============================================
    const GOOGLE_CLIENT_ID = '132734756528-ubqhpsahvhbmnbihggct1ch3ifufa3pb.apps.googleusercontent.com';
    const REDIRECT_URI = 'https://jakndklnhhaifjomebaccopfmkhiojfc.chromiumapp.org/';

    /**
     * Helper: Parse access_token from URL hash fragment
     * Handles URL encoding and edge cases
     */
    function extractTokenFromUrl(urlString) {
        try {
            // The token is in the hash fragment: #access_token=xxx&token_type=Bearer&...
    ...
    async function triggerLogin() {
        ...
        if (customGoogleLogin) {
            customGoogleLogin.innerHTML = '<span class="icon">⏳</span> Signing in...';
            customGoogleLogin.disabled = true;
        }

        try {
            // Check if running in Chrome Extension context
            if (typeof chrome !== 'undefined' && chrome.identity && chrome.identity.launchWebAuthFlow) {
                await authenticateWithChromeIdentity();
            } else {
            ...
            // Success state
            if (customGoogleLogin) {
                customGoogleLogin.innerHTML = '<span class="icon">✓</span> Signed In';
            }
            ...
            if (customGoogleLogin) {
                customGoogleLogin.innerHTML = '<span class="icon">G</span> Sign in with Google';
                customGoogleLogin.disabled = false;
            }
        }
    }

    async function authenticateWithChromeIdentity() {
        // Step A: Construct Auth URL (Implicit Grant)
        const authUrl =
        ...
        console.log('Callback URL:', responseUrl);

        // Step C: Extract access_token from hash fragment
        const accessToken = extractTokenFromUrl(responseUrl);
        ...
        // Step D: Authenticate with Firebase
    ...
    if (auth) {
        // Login Handlers
        if (customGoogleLogin) customGoogleLogin.addEventListener('click', triggerLogin);
        if (topLoginBtn) topLoginBtn.addEventListener('click', triggerLogin);
        ...
        auth.onAuthStateChanged(user => {
            ...
            if (user) {
                // Logged In
                if (customGoogleLogin) customGoogleLogin.classList.add('hidden');
                if (userProfile) {
                ...
                // Leaderboard: Hide Simulation in "Real" Mode
                const simBtn = document.getElementById('simulateLeaderboardBtn');
                if (simBtn) simBtn.style.display = 'none';
            } else {
                // Logged Out
                if (customGoogleLogin) {
                    customGoogleLogin.classList.remove('hidden');
                    customGoogleLogin.innerHTML = '<span class="icon">G</span> Sign in with Google';
                    customGoogleLogin.disabled = false;
                }
                if (userProfile) {
    ...
    // =============================================
    // CLOUD SYNC FUNCTIONS (Firestore)
    // =============================================
        ...
        docRef.get().then((doc) => {
            if (doc.exists) {
                ...
                if (cloudData.settings) {
                    applySettingsFromCloud(cloudData.settings);
                }

                console.log("✅ Synced from Cloud");
            } else {
                // No cloud data yet, save current local to cloud
     ...
     * Uses { merge: true } to avoid overwriting unrelated fields
     */
    function saveToCloud() {
        ...
        db.collection('users').doc(currentUser.uid).set(syncData, { merge: true })
    ...
    function collectCurrentSettings() {
        return {
            refreshInterval: localStorage.getItem('lofi_refresh_val') || '24',
            focusText: localStorage.getItem('lofi_focus_text') || ''
        };
    }

    /**
     * Apply settings loaded from cloud
     */
    function applySettingsFromCloud(settings) {
        if (settings.refreshInterval) {
            localStorage.setItem('lofi_refresh_val', settings.refreshInterval);
            const refreshSelect = document.getElementById('refreshInterval');
            if (refreshSelect) refreshSelect.value = settings.refreshInterval;
        }

        if (settings.focusText !== undefined) {
            localStorage.setItem('lofi_focus_text', settings.focusText);
            const taskInput = document.getElementById('taskInput');
            if (taskInput) taskInput.value = settings.focusText;
        }
    }

    /**
     * Save RPG data locally and optionally to cloud
     */
    function saveRPG(syncCloud = true) {
        localStorage.setItem('lofi_rpg', JSON.stringify(rpgData));
